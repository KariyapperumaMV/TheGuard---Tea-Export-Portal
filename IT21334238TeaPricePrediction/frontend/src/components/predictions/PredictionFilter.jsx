...
const PredictionFilter = ({ prediction, setFilteredData }) => {
    ...
    filtered = filtered.map((region) => {
      const filteredRegionData = region.data.filter((p) => {
        ...
        switch (selectedPeriod) {
          ...
          case "3months":
            return date - now <= 3 * 30 * 24 * 60 * 60 * 1000 && date >= now;
          case "6months":
            return date - now <= 6 * 30 * 24 * 60 * 60 * 1000 && date >= now;
          case "1year":
            return date - now <= 365 * 24 * 60 * 60 * 1000 && date >= now;
          default:
            return true;
        }
      });
  ...
  const allDates = prediction.data.flatMap(region => region.data.map(p => new Date(p.date)));
    ...
    <div className="mb-6">
      ...
      {isFilteringDisabled && (
        <div className="mb-4 p-3 bg-red-500 text-white rounded-md text-center">
          Time-based filtering is disabled because the prediction period has ended.
        </div>
      )}
      ...
      <div className="flex gap-4 mb-4">
        {[
          ...
          <button
            ...
            } text-white ${isFilteringDisabled ? "opacity-50 cursor-not-allowed" : "hover:bg-green-600"}`}
      ...
      <div className="mb-4">
        <label className="text-black font-semibold block mb-1">Filter by Region Category:</label>
        <select
          onChange={(e) => {
      ...
      {selectedCategory !== "all" && (
        <div className="mb-4">
          <label className="text-black font-semibold block mb-1">Select Region:</label>
          <select
          ...
          >
            ...
            {regionCategories[selectedCategory].map((region, index) => (
              <option key={index} value={region}>
                {region.replace(/_/g, " ").replace(/\b\w/g, (char) => char.toUpperCase())}
              </option>
            ))}
          </select>
        </div>
