...
import TeaPredictionList from "../predictions/TeaPredictionList";

const TeaHistoricalPriceList = () => {
    ...
    >
      <div>
        <h2 className="text-3xl font-bold text-green-600 text-center mb-2">
          Tea Historical Price Trends
        </h2>

        <hr className="border-b-1 border-green-600 mx-auto mb-6" />
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {history.map((entry) => {
            const start = new Date(entry.startDate).toISOString().slice(0, 10);
