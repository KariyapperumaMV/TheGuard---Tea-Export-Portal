...
const TeaPrice = () => {
  ...
  // fetch tea-price
  const fetchTeaDataByFilters = async (r, sr, sd, ed) => {
    setLoading(true);
    try {
      const payload = {
        TeaRegion: r,
  ...
  const saveSearchFilters = async () => {
    try {
      await axios.post(`${baseURL}teahistory`, {
        regionCategory,
  ...
  useEffect(() => {
    const init = async () => {
      if (id) {
        // load saved filters
        try {
          const { data: entry } = await axios.get(`${baseURL}teahistory/${id}`);
